...
class Enigma {
  ...
  encryptChar(c) {
    ...
    // Первое прохождение через коммутационную панель
    ...
    // Проход через роторы слева направо
    for (let i = 0; i < this.rotors.length; i++) {
      c = this.rotors[i].backward(c);
    }

    // Второе прохождение через коммутационную панель
    c = plugboardSwap(c, this.plugboardPairs);
...
// Экспортируем класс Enigma для тестов
